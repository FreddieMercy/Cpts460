...
// VRAM base address
u16 vid_mask = 0x3FFF;
// mask=Video RAM size - 1
u16 offset;
// offset from VRAM segment base
int color;
// attribute byte
int org;
// current display origin, r.e. VRAM base
int row, column;
...
{ 
	...
	if (offset <= vid_mask)
	{
		// offset still within VRAM area
		org += 2*LINE_WIDTH;
		// just advance org by ONE line
	}
